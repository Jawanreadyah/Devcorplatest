...
import React, { useRef, useState, useContext, createContext, ReactNode, useEffect } from "react";
...
// Context to manage global audio playback
const AudioPlayerContext = createContext<{
  current: string | null;
  setCurrent: (id: string | null) => void;
}>({ current: null, setCurrent: () => {} });

export function AudioPlayerProvider({ children }: { children: ReactNode }) {
  const [current, setCurrent] = useState<string | null>(null);
  return (
    <AudioPlayerContext.Provider value={{ current, setCurrent }}>
      {children}
...
const AudioPlayer = ({
  ...
  const { current, setCurrent } = useContext(AudioPlayerContext);
  const [id] = useState(() => `audio-player-${globalId++}`);
